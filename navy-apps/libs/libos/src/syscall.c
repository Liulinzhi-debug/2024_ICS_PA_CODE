...
# define RAY ("int $0x80", "eax", "ebx", "ecx", "edx", "eax")
...
void debug_sbrk(char *msg, intptr_t increment) {
    char buffer[256]; 
    int len;
    
    
    len = sprintf(buffer, "_sbrk: %s increment %d\n", msg, increment);
...
void *_sbrk(intptr_t increment) {
    intptr_t prev = brk;
    intptr_t new_brk = prev + increment; 
    intptr_t res = _syscall_(SYS_brk, new_brk, 0, 0); 
    if (res == 0) { 
        brk = new_brk;
        //debug_sbrk("Heap extended successfully", increment);
        return (void *)prev;
    } else {
        //debug_sbrk("Failed to extend heap", increment);
        return (void *)-1; 
    }
}
