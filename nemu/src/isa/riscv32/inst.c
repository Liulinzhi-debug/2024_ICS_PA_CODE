...
#define CSR(imm)  *(register_csr(imm))
#define ECALL(dnpc) { bool success; dnpc = (isa_raise_intr(isa_reg_str2val("a7", &success), s->pc)); }
enum {
  TYPE_I, TYPE_U, TYPE_S,TYPE_J,TYPE_B,TYPE_R,//TYPE_Iu,
  TYPE_N, // none
};
static word_t *register_csr(word_t imm){
  imm=(imm<<20)>>20;
   switch(imm)
   {
    case 0x180: 
    ...
    default:
    imm=(imm<<20)>>20;
    printf("illegal csr_register name 0X%x\n",imm);
    return 0;
    break;
   }
}
...
//#define immIu() do { *imm = (word_t)BITS(i, 31, 20)<<12; } while(0)    
...
INSTPAT("??????? ????? ????? 000 ????? 11001 11", jalr   , I, s->dnpc = (src1 + imm) & 0xFFFFFFFE; R(rd) = s->pc + 4;
        IFDEF(CONFIG_FTRACE, if(rd==1){call_display(s->pc, s->dnpc);}
                             else if(s->isa.inst.val == 0x00008067){ret_display(s->pc, s->dnpc);}));

INSTPAT("??????? ????? ????? ??? ????? 11011 11", jal    , J, s->dnpc = s->pc + imm; R(rd) = s->pc + 4;
                                                              IFDEF(CONFIG_FTRACE, if(rd == 1)
                                                                {
                                                                  call_display(s->pc, s->dnpc);
                                                                }));
  ...
  INSTPAT("??????? ????? ????? 000 ????? 00100 11", addi   , I, R(rd) = src1 + imm);//13 93
  ...
  INSTPAT("0100000 ????? ????? 101 ????? 00100 11", srai   , I, R(rd) = (sword_t)src1>>(word_t)(imm&0x1f));
 ...
 //INSTPAT("0000000 00000 00000 000 00000 11100 11", ecall  , I,{ bool success; s->dnpc = (isa_raise_intr(isa_reg_str2val("a7", &success), s->pc)); });
  ...
  INSTPAT("0011000 00010 00000 000 00000 11100 11", mret   , N, s->dnpc=cpu.csr.mepc; restore_interrupt() );
...
int isa_exec_once(Decode *s) {
  ...
  //IFDEF(CONFIG_ITRACE, iringbuf_get(*s));
